...
class OptionMarketMonitor(MonitorTable):
    ...
    headers: list[dict] = [
        ...
        {"name": "bid_impv", "display": "Buy hidden wave", "cell": BidCell},
        ...
        {"name": "ask_impv", "display": "Sell hidden wave", "cell": AskCell},
    ...
    def init_ui(self) -> None:
        """"""
        self.setWindowTitle("T-shaped quotation")
...
class OptionGreeksMonitor(MonitorTable):
    ...
    def init_ui(self) -> None:
        ...
        labels: list = ["Category", "Code"] + [d["display"] for d in self.headers]
        self.setHorizontalHeaderLabels(labels)

        # Init cells
        row_settings: list = []
        row_settings.append((self.portfolio_name, "Combination"))
        ...
        for underlying_symbol in underlying_symbols:
            row_settings.append((underlying_symbol, "Target"))
        row_settings.append(None)

        chain_symbols: list = list(portfolio.chains.keys())
        chain_symbols.sort()
        for chain_symbol in chain_symbols:
            row_settings.append((chain_symbol, "Option chain"))
        row_settings.append(None)

        option_symbols: list = list(portfolio.options.keys())
        option_symbols.sort()
        for option_symbol in option_symbols:
            row_settings.append((option_symbol, "Options"))
    ...
    def update_underlying_tick(self, vt_symbol: str) -> None:
        ...
        self.update_row(vt_symbol, "Target", underlying)
        ...
        for chain in underlying.chains.values():
            ...
            for option in chain.options.values():
                self.update_row(option.vt_symbol, "Options", option)

        portfolio: PortfolioData = underlying.portfolio
        self.update_row(portfolio.name, "Combination", portfolio)
    ...
    def update_pos(self, vt_symbol: str) -> None:
        ...
        if isinstance(instrument, OptionData):
            self.update_row(vt_symbol, "Options", instrument)
        else:
...
class OptionChainMonitor(MonitorTable):
    ...
    def init_ui(self) -> None:
        ...
        labels: list = ["Option chain", "Remaining trading days", "Subject matter", "Premiums and discounts"]
