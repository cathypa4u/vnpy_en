...
class ElectronicEyeAlgo:
    ...
    def start_trading(self, params: dict) -> bool:
        ...
        if not self.pricing_active:
            self.write_log("Please activate pricing first")
            return False
        ...
        self.put_status_event()
        self.write_log("Start transaction")
    ...
    def on_trade(self, trade: TradeData) -> None:
        """"""
        msg: str = (
            f"Entrusted transaction, {trade.direction} {trade.offset} {trade.volume}@{trade.price},"
            f"Order number [{trade.vt_orderid}, transaction number [{trade.vt_tradeid}]"
        )
        self.write_log(msg)
    ...
    def send_order(
        ...
        self.write_log(f"Issue an order, {direction} {offset} {volume}@{price} [{vt_orderid}]")
