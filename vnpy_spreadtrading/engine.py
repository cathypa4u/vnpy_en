...
class SpreadDataEngine:
    ...
    def process_tick_event(self, event: Event) -> None:
        ...
        for spread in self.symbol_spread_map[tick.vt_symbol]:
            # Only send event if spread quoting successfully calculated
    ...
    def process_trade_event(self, event: Event) -> None:
        ...
        # Query the spread corresponding to this trade, and update spread position calculation
    ...
    def add_spread(
        ...
        if name in self.spreads:
            self.write_log(f"Spread creation failed, name duplication: {name}")
            return
        ...
        self.write_log(f"Spread created successfully: {name}")
        self.put_data_event(spread)
    ...
    def remove_spread(self, name: str) -> None:
        ...
        self.save_setting()
        self.write_log(f"Spread removed successfully: {name}, effective after restart")
    ...
    def update_order_spread_map(self, vt_orderid: str, spread: SpreadData) -> None:
        """Update order ID corresponding spread mapping relationship"""
...
class SpreadAlgoEngine:
    ...
    def start(self) -> None:
        ...
        self.write_log("Spread Algo Engine started successfully")
    ...
    def start_algo(
        ...
        if not spread:
            self.write_log(f"Create Spread Algo failed, spread not found: {spread_name}")
            return ""
    ...
    def stop_algo(
        ...
        if not algo:
            self.write_log(f"Stop Spread Algo failed, algo not found: {algoid}")
            return
    ...
    def send_order(
        ...
        # Determine whether to use net position or lock position mode
        net: bool = not lock

        # Execute order conversion
        ...
        for req in req_list:
            ...
            # Cache relationship between order ID and spread
    ...
    def cancel_order(self, algo: SpreadAlgoTemplate, vt_orderid: str) -> None:
        ...
        if not order:
            self.write_algo_log(algo, f"Cancel order failed, order not found {vt_orderid}")
            return
...
class SpreadStrategyEngine:
    ...
    def load_strategy_class_from_module(self, module_name: str) -> None:
        ...
        try:
            ...
            msg: str = f"Strategy file {module_name} failed to load, exception triggered: \n{traceback.format_exc()}"
            self.write_log(msg)
    ...
    def add_strategy(
        ...
        if strategy_name in self.strategies:
            self.write_log(f"Create strategy failed, name duplication {strategy_name}")
            return

        strategy_class: type | None = self.classes.get(class_name, None)
        if not strategy_class:
            self.write_log(f"Create strategy failed, strategy class not found {class_name}")
            return

        spread: SpreadData | None = self.spread_engine.get_spread(spread_name)
        if not spread:
            self.write_log(f"Create strategy failed, spread not found {spread_name}")
            return
    ...
    def _init_strategy(self, strategy_name: str) -> None:
        ...
        if strategy.inited:
            self.write_log(f"{strategy_name} already initialized, repeat operation forbidden")
            return
        ...
        self.put_strategy_event(strategy)
        self.write_log(f"{strategy_name} initialization completed")
