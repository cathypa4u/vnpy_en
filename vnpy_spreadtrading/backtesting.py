...
class BacktestingEngine:
    ...
    def load_data(self) -> None:
        """"""
        self.output("Starting to load historical data")
        ...
        self.output(f"Historical data loading completed, data volume: {len(self.history_data)}")
    ...
    def run_backtesting(self) -> None:
        ...
        self.output("Starting to replay historical data")

        for data in self.history_data:
            try:
                func(data)
            except Exception:
                self.output("Exception triggered, backtesting terminated")
                self.output(traceback.format_exc())
                return

        self.output("Historical data replay ended")

    def calculate_result(self) -> DataFrame:
        """"""
        self.output("Starting to calculate daily marked-to-market Pnl")

        if not self.trades:
            self.output("Backtesting trade records are empty")
        ...
        self.output("Daily marked-to-market Pnl calculation completed")
        return self.daily_df

    def calculate_statistics(self, df: DataFrame | None = None, output: bool = True) -> dict:
        """"""
        self.output("Starting to calculate strategy statistics metrics")
        ...
        if df is not None:
            ...
            if not positive_balance:
                self.output("Margin call occurred during backtesting (Capital less than or equal to 0), unable to calculate strategy statistics metrics")
        ...
        if output:
            ...
            self.output(f"Profit Trading Days: \t{profit_days}")
            self.output(f"Loss Trading Days: \t{loss_days}")
            ...
            self.output(f"Longest Drawdown Days: \t{max_drawdown_duration}")
            ...
            self.output(f"Daily Avg Pnl: \t{daily_net_pnl:,.2f}")
            self.output(f"Daily Avg Commission: \t{daily_commission:,.2f}")
            self.output(f"Daily Avg Slippage: \t{daily_slippage:,.2f}")
            self.output(f"Daily Avg Turnover: \t{daily_turnover:,.2f}")
            self.output(f"Daily Avg Trade Count: \t{daily_trade_count}")
