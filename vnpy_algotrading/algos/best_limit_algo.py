...
class BestLimitAlgo(AlgoTemplate):
    ...
    def __init__(
        ...
        # Check max/min submission volume
        if self.min_volume <= 0:
            self.write_log("Minimum submission volume must be greater than 0, algorithm failed to start")
            self.finish()
            return

        if self.max_volume < self.min_volume:
            self.write_log("Maximum submission volume must not be less than minimum order volume, algorithm failed to start")
            self.finish()
            return
