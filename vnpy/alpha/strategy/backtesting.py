...
class BacktestingEngine:
    ...
    def set_parameters(
        ...
        for vt_symbol in vt_symbols:
            ...
            if not setting:
                logger.warning(f"The transaction configuration of contract {vt_symbol} cannot be found, please check!")
                continue
    ...
    def load_data(self) -> None:
        ...
        if empty_symbols:
            logger.info(f"Some contract historical data are empty: {empty_symbols}")

        logger.info("All historical data is loaded")
    ...
    def run_backtesting(self) -> None:
        ...
        logger.info("Start playing back historical data")
        for dt in dts:
            try:
                self.new_bars(dt)
            except Exception:
                logger.info("An exception is triggered and the backtest is terminated")
                logger.info(traceback.format_exc())
                return

        logger.info("Historical data playback ends")

    def calculate_result(self) -> pl.DataFrame | None:
        """Calculate daily mark-to-market profit and loss"""
        logger.info("Start calculating daily mark-to-market profit and loss")

        if not self.trades:
            logger.info("The transaction record is empty and cannot be calculated")
            return None
        ...
        logger.info("Daily mark-to-market profit and loss calculation completed")
        return self.daily_df

    def calculate_statistics(self) -> dict:
        """Calculate strategy statistics"""
        logger.info("Start calculating strategy statistical indicators")
        ...
        if df is not None:
            ...
            if not positive_balance:
                logger.info("There was a liquidation during the backtest (the funds were less than or equal to 0), and the strategy statistical indicators could not be calculated")
        ...
        logger.info(f"Maximum drawdown in percentage: {max_ddpercent:,.2f}%")
        logger.info(f"Maximum drawdown days: {max_drawdown_duration}")

        logger.info(f"Total profit and loss: {total_net_pnl:,.2f}")
        logger.info(f"Total commission: {total_commission:,.2f}")
        logger.info(f"Total transaction amount: {total_turnover:,.2f}")
        logger.info(f"Total number of transactions: {total_trade_count}")

        logger.info(f"Average daily profit and loss: {daily_net_pnl:,.2f}")
        logger.info(f"Average daily handling fee: {daily_commission:,.2f}")
        logger.info(f"Average daily turnover: {daily_turnover:,.2f}")
        logger.info(f"Average daily number of transactions: {daily_trade_count}")

        logger.info(f"Average daily return: {daily_return:,.2f}%")
        ...
        logger.info("Strategy statistical indicator calculation completed")
        return statistics
    ...
    def get_signal(self) -> pl.DataFrame:
        ...
        if not self.datetime:
            self.write_log("Data playback has not started and model prediction values ​​cannot be loaded")
            return pl.DataFrame()
        ...
        if signal.is_empty():
            self.write_log(f"The signal model prediction value corresponding to {dt} cannot be found")
