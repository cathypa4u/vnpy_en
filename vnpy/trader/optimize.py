...
class OptimizationSetting:
    ...
    def add_parameter(
        ...
        if end is None or step is None:
            ...
            return True, _("Fixed parameters added successfully")
...
def run_ga_optimization(
    ...
    with ctx.Manager() as manager, ctx.Pool(max_workers) as pool:
        ...
        # Run ga optimization
        output(_("Start performing genetic algorithm optimization"))
        output(_("Parameter optimization space: {}").format(total_size))
        output(_("Total number of ethnic groups per generation: {}").format(pop_size))
        output(_("Number of excellent filters: {}").format(mu))
        ...
        output(_("Genetic algorithm optimization is completed, taking {} seconds").format(cost))
