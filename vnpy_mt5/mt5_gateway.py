...
class Mt5Gateway(BaseGateway):
    ...
    def query_history(self, req: HistoryRequest) -> list[BarData]:
        ...
        if not packet or "result" not in packet:
            self.write_log("Query kline history failed: No response from server")
            return history
        ...
        if packet["result"] == -1:
            ...
            for d in packet["data"]:
                bar: BarData = BarData(
                    symbol=req.symbol.replace('.', '-'),
                    exchange=Exchange.OTC,
                    datetime=generate_korea_datetime(d["time"]),
            ...
            begin: datetime = generate_korea_datetime(data[0]["time"])
            end: datetime = generate_korea_datetime(data[-1]["time"])
...
class Mt5Client:
    ...
    def send_request(self, req: dict) -> dict:
        ...
        try:
            ...
            # 2초(2000ms) 동안만 기다림
            if self.socket_req.poll(50000):        
                data: dict = self.socket_req.recv_json()
                return data
            else:
                print(f"[Timeout] MT5 서버(Port 6888)로부터 응답이 없습니다. 방화벽이나 EA 실행 여부를 확인하세요.")
                # 소켓 재설정을 위해 재연결 로직이 필요할 수 있으나, 우선 루프 탈출
                return {}
                
        except Exception as e:
            print(f"[Error] ZMQ 통신 오류: {e}")
            return {}

def generate_local_datetime(timestamp: int) -> datetime:
    """Generate local datetime"""
    dt: datetime = datetime.fromtimestamp(timestamp)
    dt: datetime = KOREA_TZ.localize(dt)
    return dt


def generate_korea_datetime(timestamp: int) -> datetime:
    """Generate China datetime"""
    dt: dict = datetime.strptime(str(timestamp), "%Y.%m.%d %H:%M")
    utc_dt: dict = dt.replace(tzinfo=UTC_TZ)
    korea_tz: dict = utc_dt.astimezone(KOREA_TZ)
    return korea_tz
