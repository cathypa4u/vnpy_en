...
class PaperEngine(BaseEngine):
    ...
    def subscribe(self, req: SubscribeRequest, gateway_name: str) -> None:
        ...
        if original_gateway_name:
            ...
            self.write_log(f"Failed to subscribe to the market, the contract {req.vt_symbol} cannot be found")
    ...
    def send_order(self, req: OrderRequest, gateway_name: str) -> str:
        """"""
        if not req.volume:
            self.write_log("The commissioned quantity is illegal, please check")
            return ""

        contract: Optional[ContractData] = self.main_engine.get_contract(req.vt_symbol)
        if not contract:
            self.write_log(f"Delegation failed, the contract {req.vt_symbol} cannot be found")
            return ""
    ...
    def send_quote(self, req: QuoteRequest, gateway_name: str) -> str:
        ...
        contract: Optional[ContractData] = self.main_engine.get_contract(req.vt_symbol)
        if not contract:
            self.write_log(f"Quote failed, the contract {req.vt_symbol} cannot be found")
            return ""
    ...
    def check_order_valid(self, order: OrderData, contract: ContractData) -> Optional[PositionData]:
        ...
        if order.status == Status.REJECTED:
            self.write_log(f"The order was rejected because the order type {order.type.value} is not supported")
        ...
        if order.direction == Direction.LONG:
            ...
            if order.volume > available:
                order.status = Status.REJECTED
                self.write_log("The order was rejected and the liquidable position was insufficient")
            else:
